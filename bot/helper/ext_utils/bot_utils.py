...
class MirrorStatus:
    ...
    STATUS_CLONING = "Đang nhân bản...♻️"
...
def get_readable_message():
    with download_dict_lock:
        ...
        for download in list(download_dict.values()):
            INDEX += 1
            if INDEX > COUNT:
                msg += f"<b>☞ 🗃️Tên tệp :</b> <code>{download.name()}</code>"
                msg += f"\n<b>☞ 🚦Trạng thái :</b> <i>{download.status()}</i>"
                if download.status() != MirrorStatus.STATUS_ARCHIVING and download.status() != MirrorStatus.STATUS_EXTRACTING:
                    ...
                    if download.status() == MirrorStatus.STATUS_CLONING:
                        ...
                        msg += f"\n<b>☞ 📤Tải lên :</b> <code>{get_readable_file_size(download.processed_bytes())}</code> of <code>{download.size()}</code>"
                    else:
                        msg += f"\n<b>☞ 📥Tải xuống :</b> <code>{get_readable_file_size(download.processed_bytes())}</code> of <code>{download.size()}</code>"
                    msg += f"\n<b>☞ ⚡️Tốc độ :</b> <code>{download.speed()}</code>" \
                            f"\n<b>☞ ⏰Thời gian :</b> <code>{download.eta()}</code> "
                    ...
                    try:
                        ...
                        pass
                    msg += f"\n<b>☞ Huỷ bỏ ❌ :</b> <code>/{BotCommands.CancelMirror} {download.gid()}</code>"
                msg += "\n\n"
                if STATUS_LIMIT is not None:
